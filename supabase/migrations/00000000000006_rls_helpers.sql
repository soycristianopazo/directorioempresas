...
--
-- Y dentro de las policies: SIEMPRE (select auth.uid()), nunca auth.uid() a
...
as $$
  select case p_visibility
    ...
    when 'REGISTERED'   then (select auth.uid()) is not null
...
-- Estas funciones son invocables desde policies (que corren como el rol del
-- usuario) pero NO se exponen como RPC: el schema `app` no está en la lista
-- de esquemas expuestos por PostgREST.
